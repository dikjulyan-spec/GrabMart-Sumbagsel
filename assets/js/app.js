...
function renderTestimonials(rows){
  ...
  box.innerHTML=data.map(t=>`<article class="testi-card">
    <div class="stars" aria-label="Rating 5 dari 5">5/5</div>
...
function cityIcon(kota){const k=String(kota||'').toLowerCase();if(k.includes('palembang'))return 'PLG';if(k.includes('lampung'))return 'LPG';if(k.includes('jambi'))return 'JBI';if(k.includes('bengkulu'))return 'BKL';if(k.includes('pangkal')||k.includes('pinang')||k.includes('bangka'))return 'PKP';return 'GM';}
...
function createAnnouncement(rows){
  ...
  wrap.innerHTML=`<div class="announcement-card announcement-slider" role="region" aria-label="Pengumuman GrabMart"><div class="announcement-viewport">${slides}</div><div class="announcement-controls">${dots}<button class="announcement-close" type="button" aria-label="Tutup pengumuman">x</button></div></div>`;
...
function createMobileStickyCta(){
  ...
  bar.innerHTML='<a class="sticky-primary" href="daftar.html" onclick="trackDaftar(\'sticky-mobile-daftar\')">Daftar Gratis</a><a class="sticky-secondary" href="#merchant-baru" onclick="trackDaftar(\'sticky-mobile-merchant-baru\')">Merchant Baru</a>';
